...
import { Calendar } from "./calc";
import { SlideBottomSheet } from "./bot"
...
export default function test(){
    ...
    const search = (date) =>{
        fetch("",(res)=>{
            setGridList(res.GRID);
            getBottomList(date,res.GRID);
        },{
            params : {
    ...
    const bottomArea = useCallback(()=>{
        if(bottomList.length > 0){
            const bottomHtml = bottomList.map((item,idx)=>{
                <div className={"ctsScheduleItem" +("01" == item.DVCD ?" applyHome" : "")} key={idx} onClick={()=> moveApplyPage(item)}>
                    <p className="t14 gray9">{item.EVNT_NM}</p>
                    <p className="t14 gray9">{util.timeSS(item.EVNT_TIME)}</p>
                </div>
            });
        ...
        <TEST>
            <div>
                ...
                <div className="ctsLegendBottom">
                    <p className="t13 cG70">1</p>
                    <p className="t13 cG70">2</p>
                </div>
            </div>
        </TEST>
    );
