...
//  const rawHTML = `<img src="" onerror="alert('You have been hacked!');" />`;
...
.ctsCalendarWarpper { height : calc( 100% -37.4rem); overflow-x: hidden; overflow-y: scroll;}
...
@media only screen and (min-width :370px) and (min-height:720px) and (max-height :900px){
    .ctsCalendarWarpper { height :calc(100% - 43rem);}
...
.ctsCalendar .ctsWeek .ctsDate .ctsScheduleDot .ty1 { background-color  : #034f36; }
.ctsCalendar .ctsWeek .ctsDate .ctsScheduleDot .ty2 { background-color  : var(--jb-blue); }
.ctsCalendar .ctsWeek .ctsDate .ctsScheduleDot .ty3  { background-color : #613e18 }
...
const Calendar = (props) =>{ 
    ...
    useEffect(()=>{
        ...
        const cleanHtml = domPurify.sanitize(dateHtml()); // You might want to pass your markdown-converted HTML here
    ...
    const dateHtml = () =>{
        ...
        if(typeof chooseTimeDt == "undefined") setChooseTimeDt(showTimeDt);
        ...
        for(let i=0; i<day1;i++){
            ...
            dateHtmlStr += "<div class=\"ctsBlank\"></div>";
        }
    ...
    //const context = DOMPurify.sanitize(dateHtml());
    return(
        <StyledCalendar>
            <div className ="ctsCalendarWarpper">
                <div className="ctsCalendar">
                    ...
                    <div className="ctsCalendarDateArea" dangerouslySetInnerHTML={{ __html: sanitizedHtml }}></div>
                </div>
            </div>
        </StyledCalendar>
